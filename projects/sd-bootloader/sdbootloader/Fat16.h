...
class Fat16/* public Print*/ {
 public:
  ...
  /*
		Retrieve the entry index in the directory corresponding to a file
		named \a filename.

		\param fileName - name of the file to check
		\return a non-negative index in the directory if the file name exists.
		A negative number otherwise.
  */
  const int16_t getFileDirEntry(const char* fileName);
