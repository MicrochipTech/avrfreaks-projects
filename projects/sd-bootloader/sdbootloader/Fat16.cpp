...
uint8_t Fat16::make83Name(const char* str, uint8_t* name)
{
  uint8_t c;
  ...
  while ((c = *str++) != '\0') {
    if (c == '.') {
      ...
      PGM_P p = PSTR("|<>^+=?/[];,*\"\\");
      uint8_t b;
      while ((b = pgm_read_byte_far(p++))) if (b == c) return false;
 ...
 *
 *  Use Fat16::lastError to check for errors.
 */
...
void Fat16::write_P(PGM_P str) {
  for (uint8_t c; (c = pgm_read_byte_far(str)); str++) write(c);
}
//------------------------------------------------------------------------------
/**
