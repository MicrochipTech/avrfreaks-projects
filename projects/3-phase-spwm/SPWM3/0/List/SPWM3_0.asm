...
_timer3_comparea_isr:
	...
	IN   R30,SREG
	ST   -Y,R30
; 0000 00AB  static unsigned char index=0;
; 0000 00AC  if (speed>794)index+=16;
	RCALL SUBOPT_0x0
	CPI  R26,LOW(0x31B)
	LDI  R30,HIGH(0x31B)
	CPC  R27,R30
	...
	SUBI R30,-LOW(16)
	RJMP _0x14
; 0000 00AD  else if(speed>594)index+=8;
_0x3:
	RCALL SUBOPT_0x0
	CPI  R26,LOW(0x253)
	LDI  R30,HIGH(0x253)
	...
	SUBI R30,-LOW(8)
	RJMP _0x14
; 0000 00AE  else if(speed>394)index+=4;
_0x5:
	RCALL SUBOPT_0x0
	CPI  R26,LOW(0x18B)
	LDI  R30,HIGH(0x18B)
	...
	SUBI R30,-LOW(4)
	RJMP _0x14
; 0000 00AF  else if(speed>194)index+=2;
_0x7:
	RCALL SUBOPT_0x0
	CPI  R26,LOW(0xC3)
	LDI  R30,HIGH(0xC3)
...
; 0000 00B1 
; 0000 00B2  OCR1A=sine[index];
	RCALL SUBOPT_0x1
	RCALL SUBOPT_0x2
	OUT  0x2A+1,R31
	OUT  0x2A,R30
; 0000 00B3  OCR1B=sine[(unsigned char)(index+84U)];
	RCALL SUBOPT_0x1
	SUBI R30,LOW(-84)
	SBCI R31,HIGH(-84)
	RCALL SUBOPT_0x3
	...
	OUT  0x28+1,R31
	OUT  0x28,R30
; 0000 00B4  OCR1CL=sine[(unsigned char)(index+170U)];
	RCALL SUBOPT_0x1
	SUBI R30,LOW(-170)
	SBCI R31,HIGH(-170)
	RCALL SUBOPT_0x3
...
_adc_isr:
	...
	IN   R30,SREG
	ST   -Y,R30
; 0000 00B9  static unsigned int adc_temp=0;
; 0000 00BA  static unsigned char n=0;
; 0000 00BB  adc_temp+=ADCW;
	IN   R30,0x4
	IN   R31,0x4+1
...
; 0000 00BE  if(++n==64)
	...
	JMP _0xB
; 0000 00BF  {
; 0000 00C0   unsigned int temp;
; 0000 00C1   n=0;
	SBIW R28,2
;	temp -> Y+0
	LDI  R30,LOW(0)
	STS  _n_S0000001000,R30
; 0000 00C2   temp=(adc_temp>>6);
	LDS  R26,_adc_temp_S0000001000
	LDS  R27,_adc_temp_S0000001000+1
	LDI  R30,LOW(6)
	CALL __LSRW12
	ST   Y,R30
	STD  Y+1,R31
; 0000 00C3   adc_temp=0;
	LDI  R30,LOW(0)
	STS  _adc_temp_S0000001000,R30
	STS  _adc_temp_S0000001000+1,R30
; 0000 00C4   if(temp<14)
	LD   R26,Y
	LDD  R27,Y+1
	SBIW R26,14
	BRSH _0xC
; 0000 00C5   {
; 0000 00C6    TCCR1A&=~((1<<COM1A1)|(1<<COM1B1)|(1<<COM1C1));
	IN   R30,0x2F
	ANDI R30,LOW(0x57)
	OUT  0x2F,R30
; 0000 00C7    TCCR3B&=~(1<<CS00);
	LDS  R30,138
	ANDI R30,0xFE
	STS  138,R30
; 0000 00C8    ETIMSK&=~(1<<OCIE3A);
	LDS  R30,125
	ANDI R30,0xEF
	STS  125,R30
; 0000 00C9    ETIFR=(1<<OCF3A);
	LDI  R30,LOW(16)
	STS  124,R30
; 0000 00CA   }
; 0000 00CB   else
...
_0xC:
; 0000 00CC   {
; 0000 00CD    if (temp<1010)speed=temp-14;
	LD   R26,Y
	LDD  R27,Y+1
	CPI  R26,LOW(0x3F2)
	LDI  R30,HIGH(0x3F2)
...
_0x15:
	STS  _speed,R30
	STS  _speed+1,R31
; 0000 00CF    temp=(Timer_Value[speed]-1);
	LDS  R30,_speed
	LDS  R31,_speed+1
	...
	ST   Y,R30
	STD  Y+1,R31
; 0000 00D0    OCR3AH=(unsigned char)(temp>>8);
	LDD  R30,Y+1
	STS  135,R30
; 0000 00D1    OCR3AL=(unsigned char)temp;
	LD   R30,Y
	STS  134,R30
; 0000 00D2    TCCR1A|=((1<<COM1A1)|(1<<COM1B1)|(1<<COM1C1));
	IN   R30,0x2F
	ORI  R30,LOW(0xA8)
	OUT  0x2F,R30
; 0000 00D3    TCCR3B|=(1<<CS00);
	LDS  R30,138
	ORI  R30,1
	STS  138,R30
; 0000 00D4    ETIMSK|=(1<<OCIE3A);
	LDS  R30,125
	ORI  R30,0x10
	STS  125,R30
; 0000 00D5   }
...
; 0000 00DA {
_main:
; 0000 00DB  DDRB=(1<<PORTB7)|(1<<PORTB6)|(1<<PORTB5);
	LDI  R30,LOW(224)
	OUT  0x17,R30
; 0000 00DC  ADMUX=(1<<REFS0);
	LDI  R30,LOW(64)
	OUT  0x7,R30
; 0000 00DD  ADCSRA=(1<<ADEN)|(1<<ADSC)|(1<<ADIE)|(1<<ADPS2)|(1<<ADPS1);
	LDI  R30,LOW(206)
	OUT  0x6,R30
; 0000 00DE  OCR1A=sine[0];
	LDI  R30,LOW(_sine*2)
	...
	OUT  0x2A+1,R31
	OUT  0x2A,R30
; 0000 00DF  OCR1B=sine[84];
	__POINTW1FN _sine,84
	LPM  R30,Z
	LDI  R31,0
	OUT  0x28+1,R31
	OUT  0x28,R30
; 0000 00E0  OCR1CL=sine[170];
	__POINTW1FN _sine,170
	LPM  R30,Z
	STS  120,R30
; 0000 00E1  TCCR1A=(1<<WGM30);
	LDI  R30,LOW(1)
	OUT  0x2F,R30
; 0000 00E2  TCCR1B=(1<<CS00);
	OUT  0x2E,R30
; 0000 00E3  TCCR3A=(1<<WGM31)|(1<<WGM30);
	LDI  R30,LOW(3)
	STS  139,R30
; 0000 00E4  TCCR3B=(1<<WGM33)|(1<<WGM32);
	LDI  R30,LOW(24)
	STS  138,R30
; 0000 00E5  #asm("sei")
	sei
