...
;   Linear V/f curve
;
;   3 phase SPWM outputs on OCR1A,OCR1B,OCR1C V1.0 2011
...
_timer3_comparea_isr:
	...
	IN   R30,SREG
	ST   -Y,R30
; 0000 00B9  static unsigned char index=0;
; 0000 00BA  if (speed>794)index+=16;
	RCALL SUBOPT_0x0
	CPI  R26,LOW(0x31B)
	LDI  R30,HIGH(0x31B)
	CPC  R27,R30
	...
	SUBI R30,-LOW(16)
	RJMP _0x1C
; 0000 00BB  else if(speed>594)index+=8;
_0x3:
	RCALL SUBOPT_0x0
	CPI  R26,LOW(0x253)
	LDI  R30,HIGH(0x253)
	...
	SUBI R30,-LOW(8)
	RJMP _0x1C
; 0000 00BC  else if(speed>394)index+=4;
_0x5:
	RCALL SUBOPT_0x0
	CPI  R26,LOW(0x18B)
	LDI  R30,HIGH(0x18B)
	...
	SUBI R30,-LOW(4)
	RJMP _0x1C
; 0000 00BD  else if(speed>194)index+=2;
_0x7:
	RCALL SUBOPT_0x0
	CPI  R26,LOW(0xC3)
	LDI  R30,HIGH(0xC3)
...
; 0000 00C0  if(speed>=f_Base)
	...
	BRLO _0xB
; 0000 00C1  {
; 0000 00C2   OCR1A=sine[index];
	RCALL SUBOPT_0x1
	RCALL SUBOPT_0x2
	RCALL SUBOPT_0x3
; 0000 00C3   OCR1B=sine[(unsigned char)(index+84U)];
	RCALL SUBOPT_0x4
	RCALL SUBOPT_0x2
	RCALL SUBOPT_0x5
; 0000 00C4   OCR1CL=sine[(unsigned char)(index+170U)];
	RCALL SUBOPT_0x6
	STS  120,R30
; 0000 00C5  }
; 0000 00C6  else if (speed<=f_Boost)
	RJMP _0xC
_0xB:
	RCALL SUBOPT_0x0
	CPI  R26,LOW(0x60)
	...
	BRSH _0xD
; 0000 00C7  {
; 0000 00C8   OCR1A=A_calc(sine[index],A_Boost);
	RCALL SUBOPT_0x1
	RCALL SUBOPT_0x7
	RCALL SUBOPT_0x3
; 0000 00C9   OCR1B=A_calc(sine[(unsigned char)(index+84U)],A_Boost);
	RCALL SUBOPT_0x4
	RCALL SUBOPT_0x7
	RCALL SUBOPT_0x5
; 0000 00CA   OCR1CL=A_calc(sine[(unsigned char)(index+170U)],A_Boost);
	RCALL SUBOPT_0x6
	ST   -Y,R30
	LDI  R30,LOW(25)
	ST   -Y,R30
...
_0xD:
; 0000 00CD  {
; 0000 00CE   unsigned long A;
; 0000 00CF   A=((N*(unsigned long)(speed-f_Boost))/M)+A_Boost;
	SBIW R28,4
;	A -> Y+0
	LDS  R30,_speed
	LDS  R31,_speed+1
	...
	__ADDD1N 25
	CALL __PUTD1S0
; 0000 00D0   OCR1A=A_calc(sine[index],(unsigned char)A);
	RCALL SUBOPT_0x1
	RCALL SUBOPT_0x8
	RCALL SUBOPT_0x3
; 0000 00D1   OCR1B=A_calc(sine[(unsigned char)(index+84U)],(unsigned char)A);
	RCALL SUBOPT_0x4
	RCALL SUBOPT_0x8
	RCALL SUBOPT_0x5
; 0000 00D2   OCR1CL=A_calc(sine[(unsigned char)(index+170U)],(unsigned char)A);
	RCALL SUBOPT_0x6
	ST   -Y,R30
	LDD  R30,Y+1
	ST   -Y,R30
...
_adc_isr:
	...
	IN   R30,SREG
	ST   -Y,R30
; 0000 00D8  static unsigned int adc_temp=0;
; 0000 00D9  static unsigned char n=0;
; 0000 00DA  adc_temp+=ADCW;
	IN   R30,0x4
	IN   R31,0x4+1
...
; 0000 00DD  if(++n==64)
	...
	JMP _0xF
; 0000 00DE  {
; 0000 00DF   unsigned int temp;
; 0000 00E0   n=0;
	SBIW R28,2
;	temp -> Y+0
	LDI  R30,LOW(0)
	STS  _n_S0000001000,R30
; 0000 00E1   temp=(adc_temp>>6);
	LDS  R26,_adc_temp_S0000001000
	LDS  R27,_adc_temp_S0000001000+1
	LDI  R30,LOW(6)
	CALL __LSRW12
	ST   Y,R30
	STD  Y+1,R31
; 0000 00E2   adc_temp=0;
	LDI  R30,LOW(0)
	STS  _adc_temp_S0000001000,R30
	STS  _adc_temp_S0000001000+1,R30
; 0000 00E3   if(temp<14)
	LD   R26,Y
	LDD  R27,Y+1
	SBIW R26,14
	BRSH _0x10
; 0000 00E4   {
; 0000 00E5    TCCR1A&=~((1<<COM1A1)|(1<<COM1B1)|(1<<COM1C1));
	IN   R30,0x2F
	ANDI R30,LOW(0x57)
	OUT  0x2F,R30
; 0000 00E6    TCCR3B&=~(1<<CS00);
	LDS  R30,138
	ANDI R30,0xFE
	STS  138,R30
; 0000 00E7    ETIMSK&=~(1<<OCIE3A);
	LDS  R30,125
	ANDI R30,0xEF
	STS  125,R30
; 0000 00E8    ETIFR=(1<<OCF3A);
	LDI  R30,LOW(16)
	STS  124,R30
; 0000 00E9   }
; 0000 00EA   else
...
_0x10:
; 0000 00EB   {
; 0000 00EC    if (temp<1010)speed=temp-14;
	LD   R26,Y
	LDD  R27,Y+1
	CPI  R26,LOW(0x3F2)
	LDI  R30,HIGH(0x3F2)
...
_0x1D:
	STS  _speed,R30
	STS  _speed+1,R31
; 0000 00EE    temp=(Timer_Value[speed]-1);
	LDS  R30,_speed
	LDS  R31,_speed+1
	...
	ST   Y,R30
	STD  Y+1,R31
; 0000 00EF    OCR3AH=(unsigned char)(temp>>8);
	LDD  R30,Y+1
	STS  135,R30
; 0000 00F0    OCR3AL=(unsigned char)temp;
	LD   R30,Y
	STS  134,R30
; 0000 00F1    TCCR1A|=((1<<COM1A1)|(1<<COM1B1)|(1<<COM1C1));
	IN   R30,0x2F
	ORI  R30,LOW(0xA8)
	OUT  0x2F,R30
; 0000 00F2    TCCR3B|=(1<<CS00);
	LDS  R30,138
	ORI  R30,1
	STS  138,R30
; 0000 00F3    ETIMSK|=(1<<OCIE3A);
	LDS  R30,125
	ORI  R30,0x10
	STS  125,R30
; 0000 00F4   }
...
; 0000 00F9 {
_main:
; 0000 00FA  DDRB=(1<<PORTB7)|(1<<PORTB6)|(1<<PORTB5);
	LDI  R30,LOW(224)
	OUT  0x17,R30
; 0000 00FB  ADMUX=(1<<REFS0);
	LDI  R30,LOW(64)
	OUT  0x7,R30
; 0000 00FC  ADCSRA=(1<<ADEN)|(1<<ADSC)|(1<<ADIE)|(1<<ADPS2)|(1<<ADPS1);
	LDI  R30,LOW(206)
	OUT  0x6,R30
; 0000 00FD  OCR1A=sine[0];
	LDI  R30,LOW(_sine*2)
	...
	OUT  0x2A+1,R31
	OUT  0x2A,R30
; 0000 00FE  OCR1B=sine[84];
	__POINTW1FN _sine,84
	LPM  R30,Z
	LDI  R31,0
	OUT  0x28+1,R31
	OUT  0x28,R30
; 0000 00FF  OCR1CL=sine[170];
	__POINTW1FN _sine,170
	LPM  R30,Z
	STS  120,R30
; 0000 0100  TCCR1A=(1<<WGM30);
	LDI  R30,LOW(1)
	OUT  0x2F,R30
; 0000 0101  TCCR1B=(1<<CS00);
	OUT  0x2E,R30
; 0000 0102  TCCR3A=(1<<WGM31)|(1<<WGM30);
	LDI  R30,LOW(3)
	STS  139,R30
; 0000 0103  TCCR3B=(1<<WGM33)|(1<<WGM32);
	LDI  R30,LOW(24)
	STS  138,R30
; 0000 0104  #asm("sei")
	sei
...
; 0000 0109 {
_A_calc:
; 0000 010A  unsigned int sine_temp;
; 0000 010B  unsigned char sine1;
; 0000 010C  sine1=((sine<128)?~sine:sine);
	CALL __SAVELOCR4
;	sine -> Y+5
;	Ain -> Y+4
;	sine_temp -> R16,R17
...
_0x19:
	MOV  R19,R30
; 0000 010D  sine_temp=(((unsigned int)sine1)<<1);
	MOV  R30,R19
	LDI  R31,0
	LSL  R30
	ROL  R31
	MOVW R16,R30
; 0000 010E  sine_temp-=255;
	__SUBWRN 16,17,255
; 0000 010F  sine_temp*=Ain;
	LDD  R30,Y+4
	MOVW R26,R16
	LDI  R31,0
	CALL __MULW12U
	MOVW R16,R30
; 0000 0110  sine_temp>>=8;
	MOV  R16,R17
	CLR  R17
; 0000 0111  sine_temp+=255;
	__ADDWRN 16,17,255
; 0000 0112  sine_temp>>=1;
	LSR  R17
	ROR  R16
; 0000 0113  sine_temp++;
	__ADDWRN 16,17,1
; 0000 0114  sine1=(unsigned char)sine_temp;
	MOV  R19,R16
; 0000 0115  if (sine<128) sine1=~sine1;
	LDD  R26,Y+5
	CPI  R26,LOW(0x80)
	BRSH _0x1B
	COM  R19
; 0000 0116  return sine1;
_0x1B:
	MOV  R30,R19
	CALL __LOADLOCR4
	ADIW R28,6
