...
init_ext0:
	ldi	temp1,(1<<ISC01)	; setup INT0 interrupt on falling edge
	sts	EICRA, temp1
	ret
...
; The interrupt routine works by jumping to the section dictated to by the value
...
got:	st	y+,temp1	; save in ram
	...
	breq	rxD		; skip fwd so dont add RX'ed CRC bytes into local CRC
	rcall	addtoCRC
...
;So, 1st byte is the HEADER, 2nd is payload (data) length, 3rd is payload (0 to 66)
;
;==============================================================================
; State2
; 1st received byte of a block gets us here. HEADER byte.
; Timeout time for a max length message (+a little-bit extra) is 15ms @57.6K
rxA:	ldi	temp1,15
	mov	timeout,temp1	; start timeout timer (15ms) [not actualy checked anywhere yet]
...
rxB:
; this should be the actual payload count. It is from 0 to 66
	jeq	temp1,0,Zdata	; if zero length, skip to state5 (CRC)
	cpi	temp1,maxdata+1
	brsh	YY		; bad length
; count good so continue
	mov	datacount,temp1	; save count for payload receive state
...
Zdata:	ldi	state,state5	; data completed
	ldi	datacount,2	; for 2 CRC bytes reception
	rjmp	I0exit
...
rxD:	dec	datacount
	brne	I0exit		; on 1st byte, just exit
...
; here on wake-up from power-down. Change ext0 int from low-level to -ve edge
wakeup:	push	temp1
