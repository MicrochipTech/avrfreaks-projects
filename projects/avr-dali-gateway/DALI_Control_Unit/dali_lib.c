...
//! This interrupt subroutine is called every 417탎
//!
...
#define DALI_RX						((DALI_RX_PORT & (1<<DALI_RX_PIN)) == 0 ? 0 : 1)						
    ...
    switch (daliBitN)
    {
      ...
      case 62:      //stop + 23 Te (minimum time between 2 FW frames)
        ...
        if (daliRepeat == 0 && daliAnswer == 0){  //a single FW frame is sent. Busy is cleared after FWframe + 23Te
          daliBusy = 0;
          (DALI_RX_PORT |= (1<<DALI_RX_PIN)); //led red is off... debug purpose !
          dali_disable_timer();
        }
