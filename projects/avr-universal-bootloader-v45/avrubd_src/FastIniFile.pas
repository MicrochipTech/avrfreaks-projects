{*******************************************************************************
                     ...
                     Copyright © 2002-2008 Pieter Zijlstra
...
{ TFastIniFile class }
  ...
  public
    constructor Create(const FileName: string); {$IFDEF FPC} reintroduce; {$ENDIF}
    destructor Destroy; override;
...
begin
  ...
  begin
    ...
    {$IFDEF FPC}{$IFDEF ENDIAN_LITTLE} {$HINTS OFF} {$ENDIF}{$ENDIF}
      // Note: P^[0] is used as an "integer" to hold the count of items.
      L := PtrInt(P^[0]);
      Inc(L);
...
{$IFNDEF LINUX}
  CharUpperBuff(AnsiUpperCaseLookUp, 256);
{$ENDIF}
end;
