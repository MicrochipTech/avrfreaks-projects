...
*	15041801
*
*	host-side program to send two bytes of bulk data to xmega controller via USB, 
*	which then echoes them back.  Controller (device) software is xmega_bulk_usb_example.
*
*	The only difference between this host program and the one I did for the AT32UC3B
*	is that here I'm using endpoint 1, for both bulk out and bulk in.
*
*
...
*	compile with:
*
*	gcc xusbbulktest.c -o xusbbulktest -lusb-1.0
*
*
*	run with:
*
*	sudo ./xusbbulktest  x  y                where x and y are both numbers in range 0 - 255
*
*
*
...
*
*	libusb-1.0 and libusb-1.0-dev must be installed from repository (e.g. with Synaptic)
*
*	Linking libusb library as shown on compile line above is mandatory
*
*
*
*	This program requires root access; therefore either use sudo or
...
uint8_t *argv[];
{
	...
	r = libusb_bulk_transfer(dev_handle, (1 | LIBUSB_ENDPOINT_OUT), out_data, 2, &num_written, 0); // device's out endpoint is 1; we're writing 2 bytes; infinite timeout

	if(r == 0 && num_written == 2) printf("success on write\n");  //we wrote the 2 bytes successfully
	else printf("Write Error\n");

	r = libusb_bulk_transfer(dev_handle, (1 | LIBUSB_ENDPOINT_IN), in_data, 2, &num_read, 0); // device's in endpoint is 1; we're reading 2 bytes; infinite timeout

	if(r == 0 && num_read == 2) printf("success on read\n");  //we read the 2 bytes successfully
	else printf("Read Error, r = %d   num_read = %d\n", r, num_read);
