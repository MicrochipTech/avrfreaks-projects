 ...
 *		
 *		Yes, I know there's code in this header file - if this bothers you separate them out!
 *
 */
...
#define delay_time    70        // default settling time in uS after a command is sent
...
void write_4chars_xy(uint8_t x, uint8_t y, uint8_t char1, uint8_t char2, uint8_t char3, uint8_t char4)	// write 4 ascii bytes at x(0-7), y(0-63)
{
	uint16_t temp = 0;
	{
		temp |= (small_font[char1][0]);
		temp = temp * 16;
		temp |= (small_font[char2][0]);
		temp = temp * 16;
		temp |= (small_font[char3][0]);
		temp = temp * 16;
		temp |= (small_font[char4][0]);
		write_word(x, y, temp);
		temp = 0;

		temp |= (small_font[char1][1]);
		temp = temp * 16;
		temp |= (small_font[char2][1]);
		temp = temp * 16;
		temp |= (small_font[char3][1]);
		temp = temp * 16;
		temp |= (small_font[char4][1]);
		write_word(x, y+1, temp);
		temp = 0;

		temp |= (small_font[char1][2]);
		temp = temp * 16;
		temp |= (small_font[char2][2]);
		temp = temp * 16;
		temp |= (small_font[char3][2]);
		temp = temp * 16;
		temp |= (small_font[char4][2]);
		write_word(x, y+2, temp);
		temp = 0;

		temp |= (small_font[char1][3]);
		temp = temp * 16;
		temp |= (small_font[char2][3]);
		temp = temp * 16;
		temp |= (small_font[char3][3]);
		temp = temp * 16;
		temp |= (small_font[char4][3]);
		write_word(x, y+3, temp);
		temp = 0;

		temp |= (small_font[char1][4]);
		temp = temp * 16;
		temp |= (small_font[char2][4]);
		temp = temp * 16;
		temp |= (small_font[char3][4]);
		temp = temp * 16;
		temp |= (small_font[char4][4]);
		write_word(x, y+4, temp);
		temp = 0;
	}
}
