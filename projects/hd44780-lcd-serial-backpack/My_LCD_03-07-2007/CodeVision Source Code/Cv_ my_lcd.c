...
Port from ImageCraft ICCAVR v7.x to CodeVision by
John Samperi 
...
#define RXC	7       //Received comlpete flag
...
unsigned char USART_BAUD (void);

unsigned char USART_DATA = NULL;
...
void main (void) 
{
	...
	while(1) 
	{
		...
		else if (USART_DATA == REG_MODE) 	//Go into command mode
			{
...
void LCD_PutCmd (char Cmd) {
	...
	#asm
	nop // PWeh must be 230nS minimum 
	nop // nop = 67nS @ 14.7456MHz	
	nop 
	nop // Here, E = 271nS @ 14.7456MHz 
	#endasm	
	LCD_CONTROL_OUT &= ~(1<<LCD_E);
	#asm 
	nop	
...
unsigned char USART_BAUD (void) 
	...
	static unsigned  char BaudLookupTable[] = {BAUD_115200, BAUD_38400, BAUD_19200, BAUD_9600};
