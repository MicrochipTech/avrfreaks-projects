...
static const unsigned char s_sig[SIG_LEN] = 
 {'*'-' ','*'-' ','*'-' ','*'-' ','*'-' ',		// At the place of '*' enter corresponding symbols
  '*'-' ','*'-' ','*'-' ','*'-' ','*'-' ','*'-' ' };		// in the same order as in the sending module
static unsigned char s_buf[BUF_LEN]="";
static unsigned char s_f[16];
static int s_bufi, s_sigi;
static long int s_cnt;
static unsigned char s_stat;
...
char recv_serial()
{
...
#if defined(__AVR_ATmega16__)
	if ( !(UCSRA & (1<<RXC)) ) { AUX_PORT &= ~RECVLED_PIN; return s_stat; }
	b = UDR;
#elif defined(__AVR_ATtiny26__)
	if ( !USI_UART_Data_In_Receive_Buffer() ) { AUX_PORT &= ~RECVLED_PIN; return s_stat; }
	b = USI_UART_Receive_Byte();
#endif
	...
	{
		s_cnt = 0;
		if ( s_bufi-SIG_LEN>=DATA_LEN )
		{
			int i, j=0;
			unsigned char m=0;
			for ( i=0;i<16;i++ ) s_f[i] = 0;
			for ( i=0;i<DATA_LEN;i++ )
			{
				b = (s_buf[i+SIG_LEN]^(0xF0+i))^0xA0;
				if ( m<++s_f[b&0x0f] ) { m=s_f[b&0x0f]; j=b&0x0f; }
				if ( m<++s_f[((b&0xf0)>>4)] ) { m=s_f[((b&0xf0)>>4)]; j=((b&0xf0)>>4); }
			}
			s_bufi = s_sigi = 0;
			s_stat = j | 0x80;
		}
...
{
	static long int skipas=0;
	static char ls=0;
	static long int skp[4] = {0,0,0,0};
