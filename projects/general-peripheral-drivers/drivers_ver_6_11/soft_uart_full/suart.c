...
volatile unsigned char* suart_0_port = (&(SUART_0_RX_OUT_REG));
...
/*####################################################################################################*/
#if SUART_0_DYNAMIC_IO_SETTINGS == 1
void suart_0_configure_io(volatile unsigned char* port_register, unsigned char rx_pin, unsigned char tx_pin)
{
...
#if    SUART_0_USE_RS232_LOGIC == 0
 *(suart_0_port-0) |= suart_0_tx_pin;
 *(suart_0_port-1) |= suart_0_tx_pin;
 *(suart_0_port-0) |= suart_0_rx_pin;
 *(suart_0_port-1) &= (~suart_0_rx_pin);
#elif  SUART_0_USE_RS232_LOGIC == 1
 *(suart_0_port-0) &= (~suart_0_tx_pin);
 *(suart_0_port-1) |= suart_0_tx_pin;
 *(suart_0_port-0) &= (~suart_0_rx_pin);
...
#if SUART_0_WILL_USE_INTERRUPT  == 1
        SUART_0_INT_REG &= (~(1<<SUART_0_INT_BIT));
#endif
