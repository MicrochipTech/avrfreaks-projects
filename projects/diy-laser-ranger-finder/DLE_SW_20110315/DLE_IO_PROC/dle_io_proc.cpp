...
int main_loop_wait(){
    ...
    { // Incoming command from IO pipe
        ...
        dw_dev_resp_req_len = 0;
        dle_io_command_processing(&dw_dev_resp_req_len);
    ...
    else if (n_rc == WAIT_OBJECT_0 + HADLE_DEV_RESP)
    {
        ...
        if (n_rc)
        {
            // Disable response TIME OUT timer
...
int dle_io_command_processing(DWORD *pdw_response_len){
