...
type
  ...
  TfrmMain = class(TForm)
    ...
    DownloadCtrlShiftD1: TMenuItem;
    reloadfile: TMenuItem;
    ...
    procedure DownloadCtrlShiftD1Click(Sender: TObject);
    procedure reloadfileClick(Sender: TObject);
  private
...
var
  ...
  DataBuf: array[0..65536 * 16] of Byte;
  DataSize: Integer;
  Debug: Boolean = False;
  TS, TB, TE: TDateTime;
...
begin
  savecfg;
  HotKeyManager1.RemoveHotKey(24644);
end;
...
begin
  ...
  if reloadfile.Checked then
  begin
  if FileExists(ActualFile) then
  begin
    OpenFile(ActualFile);
    dlgOpen.FileName := ActualFile;
  end
  else exit;
  end;
  ...
  begin
    ...
    try
      ...
      for i := 0 to tbMenu.ButtonCount - 1 do tbMenu.Buttons[i].Enabled := False;
      for i := 0 to tbTool.ButtonCount - 1 do tbTool.Buttons[i].Enabled := False;
...
begin
  ...
  try
    ...
    if (cfg.fillunused and btnDownload.Enabled and (CompareText('.hex', ExtractFileExt(FileName)) = 0)) or (CompareText('.aub', ExtractFileExt(FileName)) = 0) then mmoBinChange(nil);
  end;
end;
...
begin
  ...
  begin
    ...
    mmoVer.Lines.Text :=
      ...
      '特别感谢' + #13#10 +
      '' + #13#10 +
      '刘海涛, 徐延康, tda1552, 倪恩伟, 程翔, Stojan Obid' + #13#10 +
      ''
      ;
    Exit;
  end;
  ...
  mmoVer.Lines.Text :=
    ...
    '' + #13#10 +
    'Liu haitao, Xu yankang, tda1552, Ni enwei, Cheng xiang, Stojan Obid' + #13#10 +
    ''
    ;
...
procedure TfrmMain.mmoLogDblClick(Sender: TObject);
begin
mmoLog.Text:='';
end;

procedure TfrmMain.HotKeyManager1HotKeyPressed(HotKey: Cardinal;  Index: Word);
begin
if HotKey = 24644 then begin if btnDownload.Enabled then btnDownload.OnClick(self) else MessageDlg('No file selected',mtWarning, [mbOk], 0); end;
end;

procedure TfrmMain.DownloadCtrlShiftD1Click(Sender: TObject);
begin
If DownloadCtrlShiftD1.Checked then DownloadCtrlShiftD1.Checked:=false else DownloadCtrlShiftD1.Checked:=true;
if DownloadCtrlShiftD1.Checked then
  begin
    if HotKeyManager1.AddHotKey(24644) = 0 then MessageDlg(HotKeyToText(24644, LOCALIZED_KEYNAMES) + ' couldn''t be assigned to a hotkey.',mtWarning, [mbOk], 0);
  end;

end;

procedure TfrmMain.reloadfileClick(Sender: TObject);
begin
if reloadfile.Checked then reloadfile.Checked := false else reloadfile.Checked := true;
end;
