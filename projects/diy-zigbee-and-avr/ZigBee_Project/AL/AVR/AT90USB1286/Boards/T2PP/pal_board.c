 ...
 */
// 9/12/2010  Copied for use as Teensy2++ Board, but no changes yet.   -jkl
// 9/13/2010  Modified so sense of LED_1-3 is active High.  - jkl
// 9/14/2010  Added uart_init so uart can be used for debug. Called from
//				pal.c pal_init().	-jkl
...
/**
 * @brief Initializes the uart by calling pal_sio_init.
 */
retval_t uart_init(void)
{
	return pal_sio_init(SIO_1);		// should check status!
}
