...
Change Lines 34 and 162 for Specific Processor
**********************************************/
#include <avr/io.h>
#include <avr/boot.h>
#include <inttypes.h>
...
void main(void)
{
	...
	ADDRESS = 0x0000;
	OSCCAL = 0xBF;				//CHANGE THIS FOR SPECIFIC PROCESSOR
	
	//Initialize Ports
	DDRD = 0x7F;		//PortD output
...
*/	
	...
	if (temp == 0x01)
	{
		...
		//Erase Flash
		for (k = 0; k < 28672 ; k +=SPM_PAGESIZE)			//CHANGE THIS FOR PROCCESSOR APPLICATION FLASH SIZE (28672 for ATmega32)			
		{
			boot_page_erase(k);
					while(boot_rww_busy())
					{
