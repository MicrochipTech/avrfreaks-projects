...
void lcdDrawBitmapWithPatternAt(uint8_t column, uint8_t line, uint8_t *bitmap, uint8_t bitmapSize, uint8_t overlappingPattern)
{
	uint8_t i;
	...
	for (i=0; i<bitmapSize; i++) 
	{
		...
		lcdWriteData(bitmap[i] | overlappingPattern, LCD_DO_NOT_FORCE_WRITE);
	}
}

void lcdDrawHorizontalPattern(uint8_t line, uint8_t pattern, uint8_t start, uint8_t end)
{
	lcdDrawBitmapWithPatternAt(start, line, &(lcdScreenData[line][start]), end-start, pattern);
}	
