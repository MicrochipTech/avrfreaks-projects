...
SetBreakHere:
		sbi DDRD, 0		; Set INT0 data direction to output
		cbi PORTD, 0	; Force INT0 low, cause an interrupt
		ret
...
; Was it external (user) or internal (code breakpoint)?
		...
		sbic DDRD, 0	; Skip if bit is clear, user's interrupt
...
isd_PCAcked:
		...
		rcall isd_Put28	; Send Size
		ldi r28, VERSION
		rcall isd_Put28	; Send version
		ldi r28, FAMILY_ATMELAVR
		rcall isd_Put28	; Send IC family code
		ldi r28, CHIP_ATMEGA103
...
isd_GotContCode:
		...
		sbi PORTD, 0	; Here for CONT Turn on INT0's Pull-Up
		cbi	DDRD, 0		; Set data direction to input
