...
Sample application program for use with the text programmer which has been  delevoped to store 
strings to flash in such a way that they are not coppied to SRAM at run-time.


Compile main program (i.e. this one) and remove the final line ":00000001FF" from the hex file.
Complile the "text programmer". Remove the last line but one ":040000030000700089" from the hex file.  
Append the text programmer hex file to the main program hex file.
Program target device with this file.

When the target device first runs program control jumps to 0x7000 (i.e. the text programmer)
This checks the flash at the start of text section
If this contains 0xFFFF it continues and requests a text file
Otherwise program control jumps to 0x0000 and the main program runs

This program contains two subroutines, one to count the number of text strings and one to print out
any given string.  The first string starts at location 0x6FFF, and occupies addresses below 0x6FFF.
The application and its text srings share the flash from addresses zero to 0x6FFF (28kB).

The appliation runs under the folowing configuration byte settings.

extended_fuse: 0xFD				Sets BOD (This is non-critical)
fuse_H:  0xD0					max boot size, resets to boot partion, WDT under program control, Ext reset enabled
fuse: 0xC2						8MHz internal RC clock


The important settings are the size of the boot partition, the reset vector and the Watch Dog Timer (WDT).
*/
...
Note: for many devices the accurracy of the internal clock is adequate and the "cal_device"  command can be ommitted.
...
newline(); 
start_address = 0x6FFF;																//start adddress of text
