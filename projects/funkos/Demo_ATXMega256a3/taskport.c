...
void Task_InitStack(TASK_STRUCT *pstTask_)
{
	...
	//!! Note !! pointers are all 16-bit.  Task functions must reside in the lower 64KWords of flash.	
	// Stack pointer is 24-bit, but we can only address the bottom 16 in our pointer.
	// This is not an issue when switching contexts later, but is a problem for
	// *starting* tasks.  Use trampolines and judicious linking to ensure that 
	// task entry functions reside in the bottom 64K words.
	
	PUSH_TO_STACK(pucStack, (UCHAR)(usAddr & 0x00FF));
	PUSH_TO_STACK(pucStack, (UCHAR)((usAddr >> 8) & 0x00FF));
	PUSH_TO_STACK(pucStack, 0x00);
	...
	// Push the additional registers required for XMEGA
	PUSH_TO_STACK(pucStack, 0x00);  // RAMPX
	PUSH_TO_STACK(pucStack, 0x00);  // RAMPY
	PUSH_TO_STACK(pucStack, 0x00);  // RAMPZ
	PUSH_TO_STACK(pucStack, 0x00);	// RAMPD
	PUSH_TO_STACK(pucStack, 0x00);	// EIND
...
{	
	...
	KernelTimer_Start();
	KernelSWI_Clear();
	ENABLE_INTS();									// Start the interrupts
...
//---------------------------------------------------------------------------
/*!
	SWI using TCC0_OVF - used to trigger a context switch
	\fn ISR(INT0_vect) __attribute__ ( ( signal, naked ) );
*/
//---------------------------------------------------------------------------
ISR(TCC0_OVF_vect) __attribute__ ( ( signal, naked ) );
ISR(TCC0_OVF_vect)
{
