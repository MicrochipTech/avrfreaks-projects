...
/* SHRINK NOTES:
  1 - Shrink code 17 Feb 19. Primary changes that need to be made is the change of pinout and different
      flash IC. Todo list:
        - Replace pinout references for new board
        - Write flash IC identification code
        - Write write/read operations for flash IC
        - Update MBC mode code (*shudders*, it's a mess)
        - Split out the MBC modes into different .S files
  2 - Completed code conversion. The erase cycle for the macronix chips is quite long. Like excessively...
      Had some initial issues getting the chips to read and write correctly, however most of these errors
      I think I can put down to poor soldering. A pins weren't connected to the pads in spite of how the
      solder job looked. Nevermind, it reads and writes without errors now. 2MB ROMs take almost 5 mins to
      program though. I'd love to speed this up considerably. I've just converted the MBC_Modes.S to have the
      correct pinout. Now for the first test game...
  3 - Found some save game issues with Conker's Pocket Tales and Pokemon Trading Card Game. Games state they are
      saving, however then don't. PTCG succesfully loads a save file from the PC, then corrupts it! I forked the
      code to investigate, and appear to have found an issue with the way double speed mode handles setting up
      the AVR for save games. This revision of code removes (NOTE - ELIMINATES!) all safeties for the RAM IC,
      however this has taken away the issue for PTCG. I haven't tried CPT again. Unfortunately, this didn't fix
      Star Wars Ep 1 Racer, which still white screens after the Nintendo logo. Not sure what the issue is there
      yet. I've also added __TIME__ and __DATE__ into the program to allow for easy recognition of which code
      revision is being run at runtime when performing a ROM info check. This revision of code is going up onto
      AVR freaks too.
*/
