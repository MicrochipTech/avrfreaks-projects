...
// Flash functions (change these names if you use a different IC - up to you to get them right though)
#define FLASH_READ_ID     mac_read_softwareID
#define FLASH_READ_BYTE   mac_flash_read_byte
#define FLASH_WRITE_BYTE  mac_flash_write_byte
#define FLASH_ERASE_CHIP  mac_flash_chip_erase
...
/*// Want to do a Flash write test here (temporarily) to diagnose where we've got issues...
  usart_transmit_str_P(PSTR("Erasing chip 0\r\n"));
  FLASH_ERASE_CHIP(0);

  usart_transmit_str_P(PSTR("Writing \"Test function - readback\"\r\n"));
  FLASH_ERASE_CHIP(0);

  char c[25] = "Test function - readback";
  uint8_t i = 0;
  FLASHADDR address;
  address.address32 = 0;

  while(c[i] != '\0') {
    mac_flash_write_byte(address, c[i]);
    i++;
    address.address32++;
  }

  usart_transmit_str_P(PSTR("Write complete. About to read IC...\r\n"));

  // Read back the data (24 chars)
  for(i = 0; i<25; i++) {
    --address.address32;
    mac_flash_read_byte(address, &c[i]);
  }

  c[24] = '\0';

  usart_transmit_str_P(PSTR("Readback:\r\n"));

  // Print the string to the terminal
  usart_transmit_str(c);

  wait_for_continue();*/
// COMMENT OUT TEST FUNCTIONALITY ABOVE

// Test code below for 32kb write and read from FRAM to prove it's the size I think it is...
/*  usart_transmit_str_P(PSTR("Confirm you wish to download a save file (Y/N)\r\n"));
  ...
  RAMaddress = 0;
  ymodem_send(sram_buff_send, "GBDump.sav", fileSize);
