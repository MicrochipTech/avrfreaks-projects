 ...
 ******************************************************************************/
 ...
 #define HSTOZGS(a) (unsigned int)(((a)<<8) | ((a)>>8))
 #define ZGSTOHS(a) HSTOZGS(a)

/******************************************************************************
 * ZG2100 macro definitions
 ******************************************************************************/
 ...
 #define ZG_MAC_TYPE_TXDATA_REQ   	            ((unsigned char) 1)
 #define ZG_MAC_TYPE_MGMT_REQ		               ((unsigned char) 2)

 #define ZG_MAC_TYPE_TXDATA_CONFIRM	            ((unsigned char) 1)
 #define ZG_MAC_TYPE_MGMT_CONFIRM	            ((unsigned char) 2)
 #define ZG_MAC_TYPE_RXDATA_INDICATE            ((unsigned char) 3)
 #define ZG_MAC_TYPE_MGMT_INDICATE	            ((unsigned char) 4)
 ...
 #define ZG_MAC_SUBTYPE_TXDATA_REQ_STD			((unsigned char)1)

 // Subtype for ZG_MAC_TYPE_MGMT_REQ and ZG_MAC_TYPE_MGMT_CONFIRM
 #define ZG_MAC_SUBTYPE_MGMT_REQ_PMK_KEY			((unsigned char)  8)
 #define ZG_MAC_SUBTYPE_MGMT_REQ_WEP_KEY			((unsigned char) 10)
 #define ZG_MAC_SUBTYPE_MGMT_REQ_CALC_PSK		   ((unsigned char) 12)
 #define ZG_MAC_SUBTYPE_MGMT_REQ_SET_PARAM	   ((unsigned char) 15)
 #define ZG_MAC_SUBTYPE_MGMT_REQ_GET_PARAM		((unsigned char) 16)
 #define ZG_MAC_SUBTYPE_MGMT_REQ_ADHOC_START		((unsigned char) 18)
 #define ZG_MAC_SUBTYPE_MGMT_REQ_CONNECT			((unsigned char) 19)
 #define ZG_MAC_SUBTYPE_MGMT_REQ_CONNECT_MANAGE	((unsigned char) 20)

// Subtype for ZG_MAC_TYPE_RXDATA_INDICATE
#define ZG_MAC_SUBTYPE_RXDATA_IND_STD			   ((unsigned char)  1)

// Subtype for ZG_MAC_TYPE_MGMT_INDICATE
#define ZG_MAC_SUBTYPE_MGMT_IND_DISASSOC		   ((unsigned char)  1)
#define ZG_MAC_SUBTYPE_MGMT_IND_DEAUTH			   ((unsigned char)  2)
#define ZG_MAC_SUBTYPE_MGMT_IND_CONN_STATUS	   ((unsigned char)  4)
...
// interrupt state
#define ZG_INTR_DISABLE		         ((unsigned char)0)
#define ZG_INTR_ENABLE		         ((unsigned char)1)
...
#define WIRELESS_MODE_INFRA	      ZG_BSS_INFRA
#define WIRELESS_MODE_ADHOC	      ZG_BSS_ADHOC
...
typedef struct
{
    unsigned char slot;	                                                   /* slot index */
    unsigned char keyLen;
    unsigned char defID;	                                                /* the default wep key id */
    unsigned char ssidLen;	                                                /* num valid bytes in ssid */
    unsigned char ssid[ZG_MAX_SSID_LENGTH];	                              /* ssid of network */
    unsigned char key[ZG_MAX_ENCRYPTION_KEYS][ZG_MAX_ENCRYPTION_KEY_SIZE];	/* wep key data for 4 default keys */
...
typedef struct
{
    ...
    unsigned char ssid[ZG_MAX_SSID_LENGTH];	            /* the string of characters representing the ssid */
    unsigned char passPhrase[ZG_MAX_WPA_PASSPHRASE_LEN]; /* the string of characters representing the passphrase */
} zg_psk_calc_req_t;
...
typedef struct
{
    unsigned char result;		         /* indicating success or other */
    unsigned char macState;	         /* current State of the on-chip MAC */
    unsigned char keyReturned;	      /* 1 if psk contains key data, 0 otherwise */
    unsigned char reserved;	         /* pad byte */
    unsigned char psk[ZG_MAX_PMK_LEN];	/* the psk bytes */
} zg_psk_calc_cnf_t;
...
typedef struct
{
    ...
    unsigned char ssid[ZG_MAX_SSID_LENGTH];
    unsigned char keyData[ZG_MAX_PMK_LEN];
} zg_pmk_key_req_t;
...
typedef struct
{
    unsigned int        rssi;                /* the value of the G1000 RSSI when the data frame was received */
    unsigned char       dstAddr[6]  ;        /* MAC Address to which the data frame was directed. */
    unsigned char       srcAddr[6];          /* MAC Address of the Station that sent the Data frame. */
    unsigned int        arrivalTime_th;      /* the value of the 32-bit G1000 system clock when the frame arrived */
    unsigned int        arrivalTime_bh;
    unsigned int        dataLen;             /* the length in bytes of the payload which immediately follows this data structure */
} zg_rx_data_ind_t;
 ...
 ******************************************************************************/
 ...
 void zg_set_ssid(unsigned char* ssid, unsigned char ssid_len);
 void zg_set_sec(unsigned char sec_type, unsigned char* sec_key, unsigned char sec_key_len);
 void zg_drv_process();
