...
int main(void)
{
	...
	while(1)
	{
		...
		if(STATE == 4)
		{
			//Fan ON/OFF state, Turn Fan on if temp exceeds threshold
			if(current > threshold)
			{
				PORTB |= 0x01;	//turn fan on
			...
			if(current <= threshold)
			{
				PORTB &= ~0x01;	//turn fan off or keep fan off
...
void PrintScreen(void)
{
	...
	LCDGotoXY(0,3);
	LCDstring(Line4,7);		//display memory usage*/
}
