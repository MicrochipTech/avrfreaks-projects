...
void vUSARTC0_Send(void *pvParameters)
{
	...
	for(;;)
	{
		...
		if (xQueueReceive( vChannelDispatchQueue, &dataByte, portMAX_DELAY ))
		{
			...
			// Do not allow a task switch until all chars of a datagram are copied to the output queue.
			// To make this fast, the capacity of the output queue must be the maximum datagram length! 
			do
			{
...
{
	const unsigned portBASE_TYPE chQueueLength       = 164;  
