...
The programer could run on an Atmega 168 but this is not recommended.  To make the
program as easy as possible to use, the strings are stored in program memory space 
...
Note: if cal bytes are stored space must also be allowed for them at the top of the EEPROM
...
#include "Full_Atmega_Programmer.h"
...
while(1){do{sendString("Y  ");timer_T1_sub(T1_delay_1sec);}								//User prompt
while((isCharavailable() == 0));															//Sends YYY to indicte activity
if ((receiveChar() == 'Y'))break;}															//Waits for user respose: Press -Y-



/*while(1){do{sendString("Z  ");timer_T1_sub(T1_delay_1sec);}									//Alternative user prompt required for						
