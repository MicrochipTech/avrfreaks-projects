...
while(1){timer_T1_sub(T1_delay_1sec); sendString("?  ");	
if (isCharavailable()) break;} 										//User prompt  
...
timer_T1_sub(T1_delay_1sec);											//Pause for optional second keypress	

if(isCharavailable()){while(1){										//If keypress made acquire it else skip to "askiX2_to_hex" subroutine
...
do 																		//Remain in do-loop untill text all downloaded
	...
	if (!(DL_status))													//If buffer full and space available in EEPROM
		...
		LED_1_off;														//and request subsequent download
		sendString("  Again!");																		
		}
...
if (DL_flag == 2)														//Download data (if any) untill EEPROM buffer is full
	...
	EEPROM_buffer)))													//Save text and data to EEPROM			
		{LED_1_off;														//If EEPROM buffer fills						
...
case 'D':																//Delete strings and data
case 'd':
...
if(target == 168){
sendHex (16, Read_write_mem('O', 0x1F7, 0));
sendChar('\t');sendHex (16, Read_write_mem('O', 0x1F8, 0));
newline();}

if(target == 328){sendHex(16, Read_write_mem('O', 0x3FE, 0));
...
switch (*ptr_DL_flag)															
	{case 2:															//Text download complete without causing EEPROM overflow 
		LED_1_on;														//Subsequent data (if any) all fits into the EEPROM_buffer
		{int n=0;\
...
case 3: 																//Similar to case 2 but the data occupies all the remaining  
	LED_1_on;															//space in the EEPROM_buffer	
	{int n=0;																																					
	while ((ADDR_last_string +n) < (EE_top))
...
case 0:																	//New file download	Contains data only
	LED_1_on;															//RAM buffer only partially full:  End of file Numbers only
	{int n=0;
