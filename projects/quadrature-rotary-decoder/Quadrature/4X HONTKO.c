...
Project                  : Distance Meter/Hontko incremental optical wireline encoder
Version                  : 120916
Date                     : 16.9.2012
Author                   : Klaus Varis
Company                  : SMOY
...
const unsigned long int ERROR_TRESHOLD = 1;
...
interrupt [EXT_INT2] void ext_int2_isr(void)
{
    ...
    lcd_init(20);
    lcd_clear();
    for (cnt = 0; cnt < 80; cnt++) lcd_buf[cnt] = 0; // Fixes randon lcd_clear issues with undesired character messy. Maybe enhanced lcd_clear function shoud be created.
    GIFR |= 0b00100000; // Clear pending EXT_INT2 interrupts caused by key debounce 
...
void main(void)
{
    ...
    while (1)
    {
        ...
        if (pulse_errors > ERROR_TRESHOLD)
        {
            ...
            sprintf(lcd_buf, "   Distance in cm\n      [%6.2f]\n\n    Errors: %lu", pulses*0.0125, pulse_errors);               
        }
