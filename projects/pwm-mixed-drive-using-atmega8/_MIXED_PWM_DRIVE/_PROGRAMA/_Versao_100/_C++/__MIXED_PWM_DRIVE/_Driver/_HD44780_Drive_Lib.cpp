...
//  |             overload after to corretly initialize display with rows and cols  |
//	| @NAME    :  Constructor    													|
//	| @INPUT   :																	|
//	| @OUTPUT  :																	|
//	| @REMARKS :  interrupts are disabled during initialization then restored		|
...
*/
//	+-------------------------------------------------------------------------------+
//	| @FUNCTION:  Prepare WIFI I/O for enter in sleep                               |
...
//	+-------------------------------------------------------------------------------+
//	| @FUNCTION:  Restore WIFI I/O from recovery of sleep state                     |
...
//	| @NAME    :  lcdSetBusyType													|
//	| @INPUT   :  type ON check busy OFF not check busy								|
//	| @OUTPUT  :  ----																|
//	| @REMARKS :  																	|
//	+-------------------------------------------------------------------------------+
void	class_HD44780_Drive::SetBusyUse(uint8_t type)
{
	BusyUse=type;
}
//	+-------------------------------------------------------------------------------+
...
//	| REMARKS :  																	|
//	+-------------------------------------------------------------------------------+
void	class_HD44780_Drive::SetBusyDelay(unsigned int time)
{
	BusyDelay=time;
}
...
//	| @NAME    :  lcdBusy															|
//	| @INPUT   :  																	|
//	| @OUTPUT  : busy ON of not busy OFF											|
//	| @REMARKS :  																	|
//	+-------------------------------------------------------------------------------+
...
//	| @INPUT   :  state OFF Invisible else Visible									|
//	| @OUTPUT  : 																	|
//	| @REMARKS :  																	|
//	+-------------------------------------------------------------------------------+
void class_HD44780_Drive::SetCursorState(uint8_t cursorState)
{
	if (cursorState!=0)
	{
...
//	| @FUNCTION:  Move Cursor to Right												|
//	| @NAME    :  lcdMoveCursorToRight												|
//	| @INPUT   : state OFF Blink Off outher value Blink is On						|
//	| @OUTPUT  : 																	|
//	| @REMARKS :  																	|
//	+-------------------------------------------------------------------------------+
void class_HD44780_Drive::SetCursorBlinkState(uint8_t blinkState)
{
	if (blinkState!=0)
	{
...
//	| @NAME    :  lcdPut															|
//	| @INPUT   :  character character to be displeyed								|
//	| @OUTPUT  : 																	|
//	| @REMARKS :  																	|
//	+-------------------------------------------------------------------------------+
...
//	| NAME    :  lcdPresence														|
//	| INPUT   :  																	|
//	| OUTPUT  :  lcd_SIZE_4_BITS or lcd_SIZE_8_BITS if display present	|
//	|	     at hardware bus else return 0					|						|
//	| REMARKS :  lcdInit must be caller before use this function					|
//	+-------------------------------------------------------------------------------+
bool class_HD44780_Drive::Present()
{
	return PresentFlag;
}
...
void	class_HD44780_Drive::PutCBig(char asciiDigit)
{
    ...
    else
    {
        ...
        if (asciiDigit>='0' && asciiDigit<='9')
        {
            ...
            PutC(pgm_read_byte(&_lcd_BigRow1[asciiDigit & 0x0f]));
            // convert ascii to number and get array index m for ROW 2
            Locate(2,Col);
            PutC(pgm_read_byte(&_lcd_BigRow2[asciiDigit & 0x0f]));
            Col++;
        }
    }
}
