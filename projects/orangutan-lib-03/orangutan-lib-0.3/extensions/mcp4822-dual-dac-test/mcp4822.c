...
//
// This uses the SPI code from the main library.  In that 
// implementation the chip select functionality is not handled by
// the SPI code.  It is up to the programmer to handle this.
// Since you might want to use more than one of these chips at once,
// this code passes the buck, too, so it's still up to the user to
// handle chip select.  The chip select is a normally high
// I/O pin that is brought low to select the SPI device of choice.
//
// For wiring, the MCP4822-to-Baby-O looks like this:
//
// MCP4822		Connect To
// 1 VDD		+5V off of a 7805
// 2 !CS		Baby-O general-purpose I/O pin (up to the user)
...
//		0 = 2x Vref
//		1 = 1x Vref
// With a gain of 0, each increment of 1 in the data increases the output
// voltage by 1mV (0.001V).  So if you send it a value of 2000, you get
// 2.000V out.  With a gain of 1, outputs are cut in half, more or less.
// Send it 2000, you get 1.000V back, and each increment of 1 in the value
// increases by half of a millivolt (0.0005V).
//
