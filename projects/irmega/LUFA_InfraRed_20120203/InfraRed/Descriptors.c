 ...
 * allowing them to retain allocated resources like COM port numbers and drivers.
 * On demos using this feature, give a warning on unsupported devices so that
 * the user can supply their own serial number descriptor instead or remove the
 * USE_INTERNAL_SERIAL value from the Device Descriptor (forcing the host to
 * generate a serial number for each device from the VID, PID and port location). */
 ...
 *  including information about any device interfaces and endpoints.
 *  The descriptor is read out by the USB host during the enumeration process
 *  when selecting a configuration so that the host may correctly communicate
 *  with the USB device. */
	...
	.Config =
		{
			...
			.ConfigAttributes       = (USB_CONFIG_ATTR_BUSPOWERED | USB_CONFIG_ATTR_SELFPOWERED | USB_CONFIG_ATTR_REMOTEWAKEUP),
 ...
 *  the manufacturer's details in human readable form, and is read out upon
