...
/************************************************************************
   ...
   This utility sends data to the IRmega hardware over virtual
   serial port. After program starts, enters the endless loop
   in which you cat specify file names and program sends them
   to IRmega. Program ends if CTRL+C pressed. 
...
int main(void) {
   ...
   while (1) { // command loop
	  ...
	  fprintf(stderr, "Enter filename to send: ");
      scanf("%s", fname);
      ...
      while ((rtn = read(dfd, &c,1))!=0) {
         if (rtn<0) {
            ...
            s[0]=c;
            fprintf(stderr, "%s", s);
            if (c>0x1F) cprintf(comport, s);  // ignore nonprintable chars
         }              
      }
      close(dfd);
      fprintf(stderr, "EOF\n");
