...
/************************************************************************
   ...
   This utility sends short pulse/space burst to the IRmega hardware
   and reeds data back. Program ends if CTRL+C pressed. 
...
int main(void) {
   ...
   char buf[4096];
   char* wptr;
   char txt[16], fname[256];
   char s[8] = "@"; 
   char c, cmd;
   int i, n, x, dfd, rtn, to;
   ...
   printf("Sending and receiving 3 pulses in loop\n\n");

   cprintf(comport, "$");       //reset buffers

   while (1) {
      cprintf(comport, "000100#"); //set watermark to 256
      cprintf(comport, "0007D0+"); //send pulse 2000 usec
      cprintf(comport, "0007D0-"); //send space 2000 usec
      cprintf(comport, "0001F4+"); //send pulse 500 usec
      cprintf(comport, "0001F4-"); //send space 500 usec
      cprintf(comport, "000064+"); //send pulse 100 usec
      cprintf(comport, "000001#"); //set watermark to 1

      data = 0; // clear data buffer
      to = 0;   // reset timeout counter

      while (to<20)  {
         n = PollComport(comport, &c, 1); // get char from IRmega
         if(n>0) {
            x = HexTab[c];
            if (x<16) {   // hex char
            ...
            zzzz(1); // ... and wait for 5 msec
         }
      } // end if no data for 100ms

      zzzz(900);  //wait for 900 msec
      printf("\nnext loop\n");
