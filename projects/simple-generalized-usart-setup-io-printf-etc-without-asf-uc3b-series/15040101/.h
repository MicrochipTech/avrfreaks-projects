...
uint8_t sr_getchar(void)
{
    if(PRINT_USART == 0)
	{
		while (!AVR32_USART0.CSR.rxrdy);		// wait for character in rx buffer
    	return AVR32_USART0.RHR.rxchr;			// return the character
	}

	else if(PRINT_USART == 1)
	{
		while (!AVR32_USART1.CSR.rxrdy);		// wait for character in rx buffer
    	return AVR32_USART1.RHR.rxchr;			// return the character
	}
...
uint8_t sr_kbhit(void)
{
	if(PRINT_USART == 0)
	{
    	if(AVR32_USART0.CSR.rxrdy) return 1;	// check if character in rx buffer; return 1 if so
		else return 0;							// otherwise return 0
	}

	else if(PRINT_USART == 1)
	{
    	if(AVR32_USART1.CSR.rxrdy) return 1;	// check if character in rx buffer; return 1 if so
		else return 0;							// otherwise return 0
	}
